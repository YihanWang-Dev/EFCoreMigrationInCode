public class MigrationEventList : IMigrationEvents
{
    ...
    public void OnColumnChanged(DbColumnInfo column, Column existing)
    {
        if (this.events != null)
        {
            foreach (var e in this.events)
            {
                e.OnColumnChanged(column, existing);
            }
        }
        Console.WriteLine($"Column {column.TableNameAndColumnName} Changed.");
    }
}
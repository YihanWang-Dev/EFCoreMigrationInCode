public abstract class MigrationEvents<T> : IMigrationEvents
{
    ...
    protected abstract void OnColumnChanged(DbColumnInfo column, Column existing);

    void IMigrationEvents.OnColumnChanged(DbColumnInfo column, Column existing)
    {
        if (column.Table.ClrType == typeof(T))
        {
            OnColumnChanged(column, existing);
        }
    }
}
...
public class Column
{
    ...
    public string EscapedColumnName { get; internal set; }
    public string TableNameAndColumnName { get; internal set; }
    public string EscapedTableNameAndColumnName { get; internal set; }


    internal bool IsSame(DbColumnInfo column)
    {
        var x = column.Property;
        if (!x.GetColumnTypeForSql().EqualsIgnoreCase(this.DataType)) return false;
